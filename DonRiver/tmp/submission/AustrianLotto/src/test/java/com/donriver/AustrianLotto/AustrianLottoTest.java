...
public class AustrianLottoTest extends TestCase{
    ...
    /**
     * The contains test cases in the order of drawing, picks and evaluation result
     * ASSUMPTION: ONE TEST CASE PER LINE
     */ 
    public void testFileTestCases() throws IllegalArgumentException, FileNotFoundException, IOException{
        AustrianLotto austrianLotto = new AustrianLotto();
        BufferedReader br = null;
        br = new BufferedReader(new FileReader("src/test/resources/testcases.txt"));
        while (true) {
        	String line = br.readLine(); //assumption: one test case per line
        	if(line == null) break;
